...
namespace AnimalRegistry
{
    public class AnimalRepository
    {
        ...
        public void TeachNewCommand(int animalId, string newCommand)
        {
            using (MySqlConnection conn = new MySqlConnection(_connectionString))
            {
                conn.Open();
                string query = "INSERT INTO Commands (AnimalId, Command) VALUES (@animalId, @command)";
                MySqlCommand cmd = new MySqlCommand(query, conn);
                cmd.Parameters.AddWithValue("@animalId", animalId);
                cmd.Parameters.AddWithValue("@command", newCommand);
                cmd.ExecuteNonQuery();
            }
        }
    }
